...
describe("JwtHandler", () => {
    describe("extractKeyId", () => {
        const jwtHandler = new JwtHandler_1.JwtHandler(debugNamePrefix, pubkeyResolver, privkeyResolver);
        ...
        it("should be rejected with MissingKeyIdError if the JWT does not contain a kid property in the header", () => {
            ...
            expect(() => jwtHandler.extractKeyId(jwtRaw)).to.throw(MissingKeyIdError_1.MissingKeyIdError);
        });
    ...
    describe("verify", () => {
        ...
        it("should return the JWT body if passed a valid JWT", (done) => {
            const jwtRaw = generateJwt(keyId, tokenBody);
            expect(jwtHandler.verify(jwtRaw)).to.eventually.deep.equal(tokenBody).notify(done);
        });
        it("should return the JWT body if passed a valid JWT and validation options that matches the JWT", (done) => {
            const jwtRaw = generateJwt(keyId, tokenBody);
            expect(jwtHandler.verify(jwtRaw, { issuer: tokenBody.iss })).to.eventually.deep.equal(tokenBody).notify(done);
        });
        it("should be rejected with JsonWebTokenError if the validation options do not match", (done) => {
            const jwtRaw = generateJwt(keyId, tokenBody);
            expect(jwtHandler.verify(jwtRaw, { issuer: "expected_issuer" })).to.eventually.rejectedWith(jwt.JsonWebTokenError).notify(done);
        });
        it("should be rejected with TokenExpiredError if the JWT is already expired", (done) => {
            ...
            expect(jwtHandler.verify(jwtRaw)).to.eventually.rejectedWith(jwt.TokenExpiredError).notify(done);
        });
        it("should be rejected with NotBeforeError if the is not yet valid", (done) => {
            ...
            expect(jwtHandler.verify(jwtRaw)).to.eventually.rejectedWith(jwt.NotBeforeError).notify(done);
        });
        it("should be rejected with JsonWebTokenError if the JWT is empty", (done) => {
            expect(jwtHandler.verify("")).to.be.rejectedWith(jwt.JsonWebTokenError).notify(done);
        });
        it("should be rejected with JsonWebTokenError if the JWT is null", (done) => {
            expect(jwtHandler.verify(null)).to.be.rejectedWith(jwt.JsonWebTokenError).notify(done);
        });
        it("should be rejected with JsonWebTokenError if the JWT is undefined", (done) => {
            expect(jwtHandler.verify(undefined)).to.be.rejectedWith(jwt.JsonWebTokenError).notify(done);
        });
        // tslint:disable-next-line:max-line-length
        it("should be rejected with MissingKeyIdError if the JWT does not contain a kid property in the header", (done) => {
            const jwtRaw = generateJwt(null, tokenBody);
            expect(jwtHandler.verify(jwtRaw)).to.eventually.rejectedWith(MissingKeyIdError_1.MissingKeyIdError).notify(done);
        });
        it("should be rejected with UnknownKeyIdError if the key id is unknown", (done) => {
            ...
            expect(jwtHandler.verify(jwtRaw)).to.eventually.rejectedWith(UnknownKeyIdError_1.UnknownKeyIdError).notify(done);
        });
        it("should be throw an Error if no pubkey resolver is specified", (done) => {
            const jwtHandlerNoPubkeyResolver = new JwtHandler_1.JwtHandler(debugNamePrefix, null, privkeyResolver);
            const jwtRaw = generateJwt(keyId, tokenBody);
            expect(jwtHandlerNoPubkeyResolver.verify(jwtRaw)).to.eventually.rejectedWith(Error).notify(done);
        });
    });
    describe("create", () => {
        const jwtHandler = new JwtHandler_1.JwtHandler(debugNamePrefix, pubkeyResolver, privkeyResolver);
        ...
        it("should be rejected with UnknownKeyIdError with a key id that does not exist", (done) => {
            ...
            expect(jwtHandler.create(tokenBody, keyIdUnknown)).to.be.rejectedWith(UnknownKeyIdError_1.UnknownKeyIdError).notify(done);
        });
        it("should be rejected with Error if no privkey resolver is specified", (done) => {
            const jwtHandlerNoPrivkeyResolver = new JwtHandler_1.JwtHandler(debugNamePrefix, pubkeyResolver, null);
            expect(jwtHandlerNoPrivkeyResolver.create(tokenBody, keyId)).to.be.rejectedWith(Error).notify(done);
        });
...
//# sourceMappingURL=JwtHandlerSpec.spec.js.map